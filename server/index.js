...
io.on("connection", function(socket) {
  ...
  socket.on(E.CHOOSE_ESER_FROM_CLIENT, async ({ id }) => {
    socket.broadcast.emit(E.CHOOSE_ESER_FROM_SERVER, { id });
    const users = await getUsersData();
  ...
  socket.on("disconnect", async function() {
    ...
    if(UID) {
      socket.broadcast.emit(E.ENABLE_ESER_FROM_SERVER, { id: UID });
      const users = await getUsersData();
      const  changedUsers = enabledUser(UID, users);
      await updateUsersData(changedUsers)
...
function disableUser(id, users) {
  return users.map(user => {
    return user.id === id ? { ...user, available: false } : user;
  });
}

function enabledUser(param) {
  return users.map(user => {
    return user.id === id ? { ...user, available: false } : user;
  });
}
